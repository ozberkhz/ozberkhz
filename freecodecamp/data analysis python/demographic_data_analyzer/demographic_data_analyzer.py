...
def calculate_demographic_data(print_data=True):
    ...
    male_employees = df[(df['sex'] == 'Male')]
    # print(male_employees.mean())
    average_age_men = round(male_employees['age'].mean(), 1)
    ...
    percentage = high_earners['native-country'].value_counts() / df['native-country'].value_counts() * 100

    highest_earning_country = percentage.idxmax()
    highest_earning_country_percentage = round(percentage.max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    indians_with_good_salary = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]
    # print(indians_with_good_salary['occupation'].value_counts())
    indian_jobs = indians_with_good_salary['occupation'].describe()
    top_IN_occupation = indian_jobs['top']
